...
use crate::utils::{download, get_extension, get_player};
use crate::{ DEFAULT_BIN_DIRECTORY};



pub fn new(version: &str) -> Result<(), Box<dyn std::error::Error>> {
    println!("{}", "> Checking available version...".purple());

    let player_info = get_player::new(version)?;
    
    let client = reqwest::blocking::Client::new();
    let res = client.get(player_info.manifest).send()?;
    ...
    if res.status().is_success() {
        ...
        println!("{}", "✓ Chalty Player installed successfully!".green());
    }
